...
public class ClientCer {
    ...
    private String pfxFileClassPath = null;
    ...
    public void setPfxFileClassPath(String pfxFile) {
        this.pfxFileClassPath = pfxFile;
    }

    public void setPfxFilePath(String pfxFile) {
        this.pfxFilePath = pfxFile;
    }
    ...
    private void init() {
        ...
        try {
            byte[] p12Cert = readFile().toByteArray();
//            byte[] p12Cert = base64Decode(pfxFile);
    ...
    public String getSelfPublickeyString(){
        init();
        return Base64.encode(selfPublicKey.getEncoded());
    }
...
//		logger("publicKey:" + base64Encoder.encode(publicKey.getEncoded()));
     ...
     * @throws Exception
     */
    private ByteArrayOutputStream readFile() throws Exception {
        InputStream fileInStream;
        if (pfxFileClassPath != null) {
            fileInStream = ClientCer.class.getResourceAsStream(pfxFileClassPath);
        }else {
            fileInStream = new FileInputStream(new File(pfxFilePath));
        }
     ...
     * @throws Exception
     */
    private byte[] sign(byte[] plainText, String signatureAlgorithms) throws UnsupportedEncodingException {

        byte[] result = "".getBytes("UTF-8");
        try {
     ...
     * @throws Exception
     */
    public byte[] sign(byte[] plainText) throws UnsupportedEncodingException {
        init();
...
//		System.out.println(params);
    ...
    public String sha1(String src) throws UnsupportedEncodingException {
